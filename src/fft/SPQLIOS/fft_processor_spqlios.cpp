...
void FFT_Processor_Spqlios::execute_reverse_int(double *res, int32_t *a) {
...
void FFT_Processor_Spqlios::execute_reverse_torus32(double *res, int32_t *a) {
...
void FFT_Processor_Spqlios::execute_direct_torus32(int32_t *res, double *a) {
    ...
    for (int32_t i = 0; i < N; i++) res[i] = int32_t(int64_t(real_inout_direct[i]));
}
